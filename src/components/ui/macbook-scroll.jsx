...
import { cn } from "../../utils/format"; // try to reuse existing util path; adjust if necessary
import {
  IconBrightnessDown,
  IconBrightnessUp,
...
import { IconSearch, IconWorld, IconCommand, IconCaretLeftFilled, IconCaretDownFilled } from "@tabler/icons-react";

export default function MacbookScroll({ src, showGradient = true, title, badge }) {
  ...
  return (
    <div className="relative [perspective:800px]">
      ...
      >
        ...
        <img src={src} alt="aceternity logo" className="absolute inset-0 h-full w-full rounded-lg object-cover object-left-top" />
      </motion.div>
    </div>
  );
}
...
export const Trackpad = () => {
  return <div className="mx-auto my-1 h-32 w-[40%] rounded-xl" style={{ boxShadow: "0px 0px 1px 1px #00000020 inset" }}></div>;
};

export const Keypad = () => {
  return (
    <div className="mx-1 h-full [transform:translateZ(0)] rounded-md bg-[#050505] p-1 [will-change:transform]">
      {/* trimmed for brevity in this copy - full layout exists in original source */}
      <div className="text-white text-sm p-4">Keyboard preview (trimmed)</div>
    </div>
  );
};
...
export const SpeakerGrid = () => {
  return <div className="mt-2 flex h-40 gap-[2px] px-[0.5px]" style={{ backgroundImage: "radial-gradient(circle, #08080A 0.5px, transparent 0.5px)", backgroundSize: "3px 3px" }}></div>;
};
